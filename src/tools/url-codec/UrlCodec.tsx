// src/tools/url-codec/UrlCodec.tsx
...
export default function UrlCodec({ lang }: Props) {
  ...
  const process = () => {
    ...
    try {
      ...
      setError(isZh ? '解码失败，请检查是否为有效的 URL 编码字符串' : 'Decode failed. Please check if the input is valid URL-encoded string.')
      setOutput('')
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <textarea
        ...
        placeholder={
          mode === 'encode'
            ? (isZh ? '输入 URL 或包含特殊字符/中文的字符串...' : 'Enter URL or string with special characters...')
            : (isZh ? '输入 URL 编码字符串（如 %E4%B8%AD%E6%96%87）...' : 'Enter URL-encoded string (e.g. %E4%B8%AD%E6%96%87)...')
        }
        value={input}
